...
## 페이지 구성
- `index.html` : 다크/라이트 토글, 전체 화면 픽셀 애니메이션 배경, 2개 메뉴 버튼
- `paint.html` : 도트 그림판
  - 연필/지우개 전환, 큐레이션 색상 팔레트 + 커스텀 색상, 브러시 두께 1~10px 세분화
  - 격자는 별도 오버레이 캔버스로 그려져 격자를 꺼도, 저장한 이미지에도 격자선이 남지 않음
  - "칸에 맞춰 채우기(스냅)" 모드로 칸 단위 채우기 가능
  - 되돌리기 / 다시실행 (버튼 또는 Ctrl+Z / Ctrl+Shift+Z)
- `game.html` : 미니게임 선택 (두더지 / 팩맨 / 체스)
- `game-mole.html` : 두더지 게임
  - 3x3/4x4/5x5 판 크기 선택, 🐹(두더지, +1점) / 💣(폭탄, -1점) 랜덤 출현
  - 남은 시간이 줄어들수록 점점 빨라짐, 최고기록 저장
- `game-pacman.html` : 팩맨 (복합 미로 + 워프 터널 + 다중 유령 + 파워펠릿 + 이동 방향에 따른 입 모양 회전, 레벨 진행, 최고기록 저장)
- `game-chess.html` : 체스
  - 정식 규칙 - 캐슬링, 앙파상, 프로모션, 체크메이트 판정 / 2인 대전 또는 AI 대전 선택, 실제 탐색 기반 AI
  - 기물은 유니코드 문자 대신 8x8 도트 비트맵 아이콘으로 렌더링
  - 프로모션 선택과 게임 중 모드 전환 확인은 브라우저 기본 팝업 대신 픽셀 스타일 모달 사용
