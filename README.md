...
- `paint.html` : 도트 그림판 (연필/지우개 전환, 큐레이션 색상 팔레트, 브러시 두께 1~10px 세분화)
- `game.html` : 미니게임 선택 (두더지 / 팩맨 / 체스)
- `game-mole.html` : 두더지 게임 (3x3/4x4/5x5 판 크기 선택, 이모지 팝업, 최고기록 저장)
- `game-pacman.html` : 팩맨 (복합 미로 + 워프 터널 + 다중 유령 + 파워펠릿 + 이동 방향에 따른 입 모양 회전, 최고기록 저장)
- `game-chess.html` : 체스 (정식 규칙 - 캐슬링, 앙파상, 프로모션, 체크메이트 판정 / 2인 대전 또는 AI 대전 선택, 실제 탐색 기반 AI)
